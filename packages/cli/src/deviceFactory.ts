...
async function getRpi7in5Bc(orientation?: Orientation, colorMode?: ColorMode): Promise<DisplayDevice> {
    try {
        const { Rpi7In5BC } = await import('@epaperjs/rpi-7in5-bc');
        return new Rpi7In5BC(orientation, colorMode);
    } catch (e) {
        console.log(e);
        throw new Error('Failed to import @epaperjs/rpi-7in5-bc, make sure it is installed');
    }
}
